...
public class Operat {
    ...
    public void caozuo(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        req.setCharacterEncoding("UTF-8");
        resp.setContentType("text/html; charset=UTF-8");
        LoginModel login = (LoginModel) req.getSession(false).getAttribute("LOGINMODEL");
        String zid = req.getParameter("id");//获取基本信息表的id
        System.out.println("id++++++++++++++:" + zid);
        String nodestatus2 = req.getParameter("nodestatus");
        System.out.println("获取nodestatus++++++++++++++:" + nodestatus2);
        String xq = req.getParameter("xq");
        System.out.println("获取nodestatus++++++++++++++:" + xq);
		String taskstage = req.getParameter("taskstage");
          ...
          //获取是否为二次办理
					...
					if(nodeStatus == 7 || nodeStatus == 8 || nodeStatus == 9 || nodeStatus == 10){//科室办理阶段
						if(i != 3 && i != 4 && i != 5 && i != 6){//不查询社区办理阶段信息
							...
							if(i>=3 && i<11 && bl != null && !"".equals(bl)){//查询办理阶段信息
								msql = "select b.*,h.*,h.ATTACH hattach,h.REPLY hreply,h.SATISFACTION hsatisfaction,h.RESOLVE hresolve from TELEPHONE_BASIC b,TELEPHONE_HANDLE h  where b.id = " + zid + " and h.hid =  b.id and nodestatus = '" + i + "' and BL = '"+deptName+"' and ISDELETE = '0'";
							}else{//不是办理阶段
								msql = "select b.*,h.*,h.ATTACH hattach,h.REPLY hreply,h.SATISFACTION hsatisfaction,h.RESOLVE hresolve from TELEPHONE_BASIC b,TELEPHONE_HANDLE h  where b.id = " + zid + " and h.hid =  b.id and nodestatus = '" + i + "' and ISDELETE = '0'";
							}
							list = dbo.prepareQuery(msql, null);
							if(list.size()>0){
								map.put("checkidea"+i, list.get(0).get("CHECKIDEA"));
								map.put("checkname"+i, list.get(0).get("CHECKNAME")+new Date().getTime());
            ...
            if(again.equals("1")){//二次办理详情查询
            	for(int i=17;i<=18;i++){//查询主管副职和社区理长的详情信息
					String msql = "select b.*,h.*,h.ATTACH hattach,h.REPLY hreply,h.SATISFACTION hsatisfaction,h.RESOLVE hresolve from TELEPHONE_BASIC b,TELEPHONE_HANDLE h  where b.id = " + zid + " and h.hid =  b.id and nodestatus = '"+i+"' and ISDELETE = '0'";
					list = dbo.prepareQuery(msql, null);
					if(list.size()>0){
						map.put("checkidea"+i, list.get(0).get("CHECKIDEA"));
