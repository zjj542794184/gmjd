...
public class MoonSummaryService {
    public JSON MoonSummaryServices(String operation,String year,String month) {
        ...
        if(operation.equals("")){
            ...
            month_now = c.get(Calendar.MONTH)+1;
        }else if(operation.equals("prev-month")){
            year_now =Integer.parseInt(year);
            month_now = Integer.parseInt(month);
            if(month_now<=1){
                year_now=year_now-1;
                month_now=12;
            }else{
                year_now= Integer.parseInt(year);
                month_now=Integer.parseInt(month)-1;
            }
        }else if(operation.equals("next-month")){
            year_now =Integer.parseInt(year);
            month_now = Integer.parseInt(month);
            if(month_now>=12){
                year_now=year_now+1;
        ...
        date =year_now+"-"+month_now+"-"+ c.get(Calendar.DATE);
        ...
        if(nowDate.after(endDate)){
            ...
            if(month_now==1||
                    ...
                    month_now==10||
                    month_now==12){
                c.set(Calendar.DAY_OF_MONTH, 51);
            }else if(month_now==4||
                    month_now==6||
                    month_now==9||
                    month_now==11){
                c.set(Calendar.DAY_OF_MONTH, 50);
                    ...
                    &&month_now==2){
                c.set(Calendar.DAY_OF_MONTH, 49);
            }else if(year_now % 4!=0
                    &&month_now==2){
                c.set(Calendar.DAY_OF_MONTH, 48);
            }
            ...
            if(
                    ...
                    month_now==12){
                c.set(Calendar.DAY_OF_MONTH, 50);
            }else if(month_now==1|| month_now==2||month_now==4||
                    month_now==6||
                    month_now==8||
                    month_now==9||
                    month_now==11){
        ...
        String sql_type =
                "select * from (select i.*,NVL2(j.ggfwl,j.ggfwl,'公共服务类') ggfwl,NVL2(j.countg,j.countg,0) countg from (\n" +
                        ...
                        "and b1.CREATETIME between to_date('"+arr[0]+"  12:00:00' ,'yyyy-mm-dd hh24:mi:ss')and  " +
                        "to_date('"+arr[1]+"   12:00:00','yyyy-mm-dd hh24:mi:ss')\n" +
                        "  AND H1.NODESTATUS='4' AND H1.ISDELETE='0' AND   B1.ID = H1.HID  group by h1.bl, b1.PROBLEMTYPE) f on e.HANDLEDEPTNAME=f.HANDLEDEPTNAME) g left join \n" +
...
//
        ...
        time.put("time", "此表统计时间为 "+ arr[0]+"至"+arr[1]);
